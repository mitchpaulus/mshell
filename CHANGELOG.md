...
- Startup now loads both `std.msh` and `init.msh` from version directories (`msh/<version>/...`), requires the versioned init file to exist, re-execs `VER` scripts with `msh-<version>` when needed, and ignores `MSHSTDLIB`/`MSHINIT` for `VER` scripts.
