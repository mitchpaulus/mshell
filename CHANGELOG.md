...
- `concat` (stdlib) now folds with `extend` instead of `+`, so it appends into a single seed list instead of allocating a new list at each step. Reduces total heap allocations by ~22% on list-concatenating workloads.
