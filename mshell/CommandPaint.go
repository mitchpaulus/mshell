...
// resizeCommandRegion keeps the prompt and command where they are after a
// terminal size change and repaints them at the new width on the next frame.
// A new pane reports its parent's size until the first key arrives, and a
// user resize must not print another prompt either. Rows above may have
// scrolled, so the absolute origin is dropped; painting only needs relative
// rows. A frame taller than the new screen is clamped to what still fits.
...
func (state *TermState) refreshInteractiveDisplay(renderHistory bool) error {
	...
	if Cells(state.numCols) != state.commandRegion.Columns || state.numRows != state.commandRegion.ScreenRows {
		state.Logf("Terminal resized %dx%d -> %dx%d; repainting in place\n", previousCols, previousRows, state.numCols, state.numRows)
		state.resizeCommandRegion(Cells(state.numCols), state.numRows)
	}
