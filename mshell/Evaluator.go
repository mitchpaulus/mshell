...
// Returns four objects from the stack.
// obj1, obj2, obj3, obj4 := stack.Pop4(t)
// obj1 was on top, obj4 is at the bottom.
func (objList *MShellStack) Pop4(t Token) (MShellObject, MShellObject, MShellObject, MShellObject, error) {
	obj1, obj2, obj3, err := objList.Pop3(t)
	if err != nil {
		return nil, nil, nil, nil, err
	}

	obj4, err := objList.Pop()
	if err != nil {
...
func (state *EvalState) processToken(token MShellParseItem, frame *EvaluationFrame, base int) *EvalResult {
	...
	case *MShellParsePrefixQuote:
		...
		funcToken := Token{
			...
			Column: t.StartToken.Column,
			Start:  t.StartToken.Start,
	...
	case MShellVarstoreList:
		return state.processVarstoreList(t, frame)
...
func (state *EvalState) evaluateBuiltinToken(t Token, stack *MShellStack, context ExecuteContext, definitions []MShellDefinition, callStackItem CallStackItem) EvalResult {
			if t.Type == EOF {
				...
				case "completeCommand":
					...
					prefix, err := prefixObj.CastString()
					if err != nil {
						return state.FailWithMessage(fmt.Sprintf("%d:%d: 'completeCommand' expected the word being completed to be a string, got a %s.\n", t.Line, t.Column, prefixObj.TypeName()))
					}
					argsObj, err := stack.Pop()
					if err != nil {
						return state.FailWithMessage(fmt.Sprintf("%d:%d: Cannot do 'completeCommand' operation on a stack with only one item.\n", t.Line, t.Column))
					}
					argsList, ok := argsObj.(*MShellList)
					if !ok {
						return state.FailWithMessage(fmt.Sprintf("%d:%d: 'completeCommand' expected a list of arguments, got a %s.\n", t.Line, t.Column, argsObj.TypeName()))
					}
					args := make([]string, len(argsList.Items))
					for i, item := range argsList.Items {
						args[i], err = item.CastString()
						if err != nil {
							return state.FailWithMessage(fmt.Sprintf("%d:%d: 'completeCommand' expected string arguments, got a %s.\n", t.Line, t.Column, item.TypeName()))
						}
					}
				...
				case "gridMeta":
					...
					if grid.Meta != nil {
						stack.Push(&Maybe{obj: grid.Meta})
					} else {
						stack.Push(&Maybe{obj: nil})
					}
				case "gridColMeta":
				...
				case "gridCompact":
					...
					switch objTyped := obj.(type) {
					case *MShellGrid:
						// Already a grid, just push it back
						stack.Push(objTyped)
					case *MShellGridView:
						// Create a new grid from the view
						newGrid := NewGrid()
						newGrid.Meta = objTyped.Source.Meta
						newGrid.RowCount = len(objTyped.Indices)

						for _, srcCol := range objTyped.Source.Columns {
							newCol := NewGridColumn(srcCol.Name, newGrid.RowCount)
							newCol.Meta = srcCol.Meta
							for i, srcIdx := range objTyped.Indices {
								newCol.GenericData[i] = srcCol.Get(srcIdx)
							}
							newGrid.AddColumn(newCol)
						}
				...
				case "gridAddCol":
					...
					if list, ok := obj1.(*MShellList); ok {
						...
						// Use obj1 as default value for all rows
						for i := 0; i < grid.RowCount; i++ {
							newCol.GenericData[i] = obj1
						}
					}
				...
				case "gridRemoveCol":
					...
					grid.Columns = append(grid.Columns[:colIdx], grid.Columns[colIdx+1:]...)

					// Rebuild ColIndex
					grid.ColIndex = make(map[string]int)
					for i, col := range grid.Columns {
						grid.ColIndex[col.Name] = i
					}
				...
				case "gridRenameCol":
					...
					// Rename column
					grid.Columns[colIdx].Name = newName
				...
				case "map":
					...
					case *MShellGrid, *MShellGridView:
						...
						if len(mappedRows) == 0 {
							...
							var colNames []string
							switch firstRow := mappedRows[0].(type) {
							case *MShellGridRow:
								for _, col := range firstRow.Grid.Columns {
									colNames = append(colNames, col.Name)
								}
							case *MShellDict:
								for k := range firstRow.Items {
							...
							newGrid := NewGrid()
							newGrid.Meta = sourceGrid.Meta
							newGrid.RowCount = len(mappedRows)
							...
							// Fill in values
							for rowIdx, row := range mappedRows {
