...
func (def *MShellDefinition) ToJson() string {
	return fmt.Sprintf("{\"name\": \"%s\", \"items\": %s, \"type\": %s }", def.Name, ToJson(def.Items), sigToJson(def.Inputs, def.Outputs))
}

func sigToJson(inputs, outputs []MShellParseItem) string {
	return fmt.Sprintf("{\"input\": %s, \"output\": %s}", ToJson(inputs), ToJson(outputs))
}
